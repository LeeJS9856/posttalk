...
const PhotoPreviewCarousel = ({ images }: PhotoPreviewCarouselProps): React.JSX.Element => {
  ...
  const currentAspectRatio = aspectRatios[currentIndex] ?? 1;
  ...
  const handleImageLoad = (index: number, event: SyntheticEvent<HTMLImageElement>): void => {
    const { naturalHeight, naturalWidth } = event.currentTarget;
    if (!naturalWidth || !naturalHeight) return;

    const aspectRatio = naturalWidth / naturalHeight;
    setAspectRatios((current) => current[index] === aspectRatio ? current : { ...current, [index]: aspectRatio });
  };
      ...
      <Track $currentIndex={currentIndex}>
        {images.map((image, index) => (
          <Slide key={image} aria-hidden={index !== currentIndex}>
            <img src={image} alt={`임시 사진 광고 ${index + 1}`} draggable={false} onLoad={(event) => handleImageLoad(index, event)} />
          </Slide>
        ))}
      </Track>
...
const Carousel = styled.div<{ $aspectRatio: number }>`
  position: relative;
  width: 100%;
